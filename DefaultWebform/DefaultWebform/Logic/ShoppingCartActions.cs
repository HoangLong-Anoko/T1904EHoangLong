...
using System.Web;
using DefaultWebform.Models;
namespace DefaultWebform.Logic
{
    public class ShoppingCartActions : IDisposable
    {
        ...
        CartItemUpdates)
        {
            using (var db = new DefaultWebform.Models.ProductContext())
            {
                try
                {
        ...
        {
            using (var _db = new DefaultWebform.Models.ProductContext())
            {
                try
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == removeCartID && c.Product.ProductID == removeProductID select c)
                                  .FirstOrDefault();
                    if (myItem != null)
                    {
                        // Remove Item.
        ...
        quantity)
        {
            using (var _db = new DefaultWebform.Models.ProductContext())
            {
                ...
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == updateCartID && c.Product.ProductID == updateProductID select c)
                                  .FirstOrDefault();
                    if (myItem != null)
                    {
